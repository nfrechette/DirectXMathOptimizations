...
// aligned constants
extern ScalarSinConstants_V04 SCALAR_SIN_CONSTANTS_V04;

// V04 is the best I could do with C++ scalar math
...
__declspec(noinline) float XMScalarSin_V04(float Value)
{
	...
	if (y > SCALAR_SIN_CONSTANTS_V04.half_pi)
	{
		y = SCALAR_SIN_CONSTANTS_V04.pi - y;
	}
	else if (!(SCALAR_SIN_CONSTANTS_V04.neg_half_pi < y))
	{
		y = SCALAR_SIN_CONSTANTS_V04.neg_pi - y;
	}

	// 11-degree minimax approximation
	float y2 = y * y;
	float result = y2 * SCALAR_SIN_CONSTANTS_V04.coefficients[0];
	result += SCALAR_SIN_CONSTANTS_V04.coefficients[1];
	result *= y2;
	result -= SCALAR_SIN_CONSTANTS_V04.coefficients[2];
	result *= y2;
	result += SCALAR_SIN_CONSTANTS_V04.coefficients[3];
	result *= y2;
	result -= SCALAR_SIN_CONSTANTS_V04.coefficients[4];
	result *= y2;
	result += SCALAR_SIN_CONSTANTS_V04.coefficients[5];
	return result * y;
}
