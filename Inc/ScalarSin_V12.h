...
constexpr uint32_t SIGN_BIT_MASK_V12 = 0x80000000;
constexpr XMVECTOR XM_HALF_V12 = { 0.5f, 0.5f, 0.5f, 0.5f };
constexpr XMVECTOR XM_PI_V12 = { XM_PI, XM_PI, XM_PI, XM_PI };
constexpr XMVECTOR XM_2PI_V12 = { XM_2PI, XM_2PI, XM_2PI, XM_2PI };
constexpr XMVECTOR XM_1DIV2PI_V12 = { XM_1DIV2PI, XM_1DIV2PI, XM_1DIV2PI, XM_1DIV2PI };
constexpr XMVECTOR XM_PIDIV2_V12 = { XM_PIDIV2, XM_PIDIV2, XM_PIDIV2, XM_PIDIV2 };
...
__declspec(noinline) float XMScalarSin_V12(const float Value)
{
	const XMVECTOR sign_bit_mask = _mm_broadcast_ss((float*)&SIGN_BIT_MASK_V12);

	XMVECTOR value_v = _mm_set_ps1(Value);
	XMVECTOR value_sign = _mm_and_ps(value_v, sign_bit_mask);

	// Map Value to y in [-pi,pi], x = 2*pi*quotient + remainder.
	XMVECTOR quotient = _mm_mul_ss(value_v, XM_1DIV2PI_V12);
	// If Value >= 0.0, our rounding offset is 0.5, otherwise -0.5, same sign as Value
	XMVECTOR rounding_offset = _mm_or_ps(value_sign, XM_HALF_V12);
	quotient = _mm_add_ss(quotient, rounding_offset);
	quotient = _mm_round_ss(quotient, quotient, _MM_FROUND_TO_ZERO);

	XMVECTOR y = _mm_sub_ss(value_v, _mm_mul_ss(quotient, XM_2PI_V12));
	...
	const XMVECTOR y_abs_le_halfpi = _mm_cmple_ss(y_abs, XM_PIDIV2_V12);
	// If |y| > pi/2, offset = y_sign * XM_PI, else offset = 0.0
	const XMVECTOR offset = _mm_or_ps(y_sign, XM_PI_V12);
